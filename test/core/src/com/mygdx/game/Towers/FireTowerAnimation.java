    ...
    public void spawnProjectile() {
        for (Slime s : PlayScreen.slimes) {
            if (hitBox.intersects(s.getHitBox())) {
                Fire fire = new Fire((float) this.getX() - (float) this.getRadius(), (float) this.getY() - (float) this.getRadius(), 100, 100);
                fires.add(fire);
                fire.create();
//                s.health -= 20;
//                if (s.health <= 0) {
...
//                }
        ...
        for (int f=0; f < fires.size(); f++) {
            if (fires.get(f).hitBox.intersects(fires.get(f).getClosest().getHitBox())) {
